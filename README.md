...
* **طبقة التحقق والتطبيع** – توفر الكلاسات `InvoiceValidator` و `InvoiceAdapter` إمكانية التحقق من اكتمال البيانات وتطبيع الحقول قبل إنشاء الفاتورة. يساعد ذلك على ضمان جودة البيانات المرسلة وتوحيد أسماء الحقول القادمة من أنظمة مختلفة.

* **معالجة الأخطاء** – يتضمن الكلاس `ZatcaErrorHandler` دعمًا لالتقاط الأخطاء الواردة من واجهة ZATCA وتحويلها إلى استثناءات يمكن التعامل معها. يمكن تعيين رد نداء (callback) فى التهيئة لمعالجة الأخطاء أو تسجيلها قبل رفع الاستثناء.
...
للاطلاع على مثال كامل يوضح كيفية إعداد الحزمة واستخدامها فى سيناريوهات متعددة (بيئة PHP خام، Laravel، فواتير ضريبية ومبسطة، دعم شركة واحدة أو عدة شركات)؛ قم بفتح الملف التالي:

* **Usage Guide PDF** – ملف PDF يشرح خطوات الاستخدام بالتفصيل، موجود فى مجلد `docs` داخل الحزمة. يمكنك الوصول إليه مباشرة عبر هذا الرابط:
  [فتح دليل الاستخدام](docs/usage_guide.pdf)

يحتوى الدليل على شرح خطوة بخطوة لتثبيت الحزمة، إعداد الشركات المتعددة، تطبيع البيانات، التحقق من الحقول، إنشاء الفواتير بأنواعها المختلفة، وطريقة التعامل مع الأخطاء.
